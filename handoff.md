...
5. 部署新版 `index.html`、`firebase-v1685.js` 到 GitHub Pages；`?v=1918` 會強制載入新版登入程式。
...
目前 v1.918 已將手機 Google 登入改為點擊後立即開啟 popup，且不再 fallback 到跨站 redirect；題庫載入仍維持 bundle 優先。
下一步請先部署 `index.html` 與 `firebase-v1685.js` 並以 iPhone 實機驗證，再處理 Firestore rules、GAS 同步與 Firebase reads 觀察。
