...
- [x] v1.937 學生視角完成度欄支援由低到高／由高到低排序。
- [x] v1.937 學生視角尚缺單元改顯示數量，點擊後才顯示完整清單。
- [x] v1.937 搜尋學生新增核取式複選，支援全選、清除及任意多人，並同步套用摘要、雙視角與最近紀錄。
- [x] v1.937 單元三種狀態人數可開啟學生名單，並新增全看板姓名遮蔽切換。
- [x] v1.937 最近 100 筆紀錄跟隨單元與學生搜尋條件，所有新操作不增加 Firestore reads。
- [x] v1.937 修正單元排行因學生列漏帶 `topicResults` 而使完成度看板更新失敗。
