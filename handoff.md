...
- [ ] v1.928 需在 iPhone Safari／Chrome 實機確認歷次成績表格可橫向滑動，長單元名稱完整換行。
...
目前 v1.927 已在本機完成歷次成績清單與明細的長單元名稱雙行顯示，並保留 v1.926 的依題數倒數與逾時自動交卷；Firestore 資料結構仍沿用 v1.925。
下一步請依 README 更新前端與 GAS，並以 iPhone 實機驗收；v1.925 歷史摘要回填工作仍依原規劃執行。
