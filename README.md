...
版本：v1.937
日期：2026-08-19
重點：完成度看板新增學生／單元雙視角、單元完成率排行與全域姓名遮蔽。
```

## v1.937 完成度雙視角與姓名遮蔽

- 完成度看板可在學生視角與單元視角間即時切換，兩者共用科目、複選單元、學生搜尋及摘要卡。
- 學生視角的「完成度」欄可點擊切換低到高／高到低排序，班級與學號作為同完成度時的穩定排序依據。
- 學生視角的「尚缺單元」欄只顯示缺少數量，點擊數字後才開啟完整單元清單，避免多選時表格被長文字撐高。
- 學生篩選使用與單元相同的核取式複選器，支援全選、清除及任意多人，並可用姓名／學號／班級文字搜尋再次縮小範圍；摘要、雙視角與最近紀錄共用相同條件。
- 單元視角依完成率排行，預設由低至高且可反轉；已完成、練習中、尚未開始人數可點開查看學生名單。
- 看板姓名預設遮蔽，中間以 `O` 取代；教師可切換完整姓名，設定同步套用學生表、單元名單與最近 100 筆紀錄。
- 最近作答紀錄會跟隨目前的單元複選與學生搜尋條件，但切換視角或姓名不重新讀取資料。
- 單元排行保留每位學生的逐單元計算結果，缺漏時安全視為尚未開始，避免排行背景運算中斷學生視角。
- 全部新增操作只使用瀏覽器既有資料，不增加 GAS 請求或 Firestore reads；GAS API 與部署版本維持 v1.936。

## v1.936 完成度單元複選

- 單元欄改為核取式複選器，支援全選、清除、單選及任意多選，不需按 Ctrl／Command。
- 切換科目時預設全選該科目目前設定單元；按清除後會提示至少選擇一個單元。
- 完成度分母、已完成／練習中／尚未練習摘要及「尚缺單元」均依目前複選集合重算。
- 只選一個單元時顯示該單元最高有效分數；複選時顯示達標單元數／選取單元數。
- 單元複選只操作瀏覽器記憶體，不增加 GAS 請求或 Firestore reads。

## v1.935 完成度 GAS 安全代理

- 管理員登入成功後由 GAS 核發 6 小時滑動期限 session；除登入外的管理 actions 均驗證 token。
- Token 只放在 POST body 與瀏覽器 `sessionStorage`，登出時由 GAS 撤銷；每帳號 10 分鐘最多 5 次失敗登入。
- 完成度看板不再要求 Firebase Google 登入，也不再由瀏覽器直接讀取全體 `studentProgress`。
- GAS 使用服務帳號與 field mask 讀取完成度必要欄位，整理成精簡摘要後回傳；不修改學生歷史資料。
- 一般進入使用 120 秒共用快取，「強制更新」才繞過快取；畫面顯示 reads、來源文件數與傳輸 KB。
- Ch08 使用 `IMM-CH08-01` 至 `IMM-CH08-08` 相容碼，管理端不再因重複 topicId 配錯成績。
- 完成度列表新增「尚缺單元」，摘要卡會跟隨搜尋條件重新計算。
- 最近 100 筆做答使用獨立懶載入代理與快取；只傳摘要、不傳 `detailsJson`，未展開時不產生其 100 筆 reads。
- 即時成績與分析頁仍維持原 Firebase 流程及 Firebase 教師權限。
- 詳細資料流、流量模型與發布順序見 `ADMIN_GAS_PROXY_V1935.md`。

## v1.934 Ch08 顯示相容修正

- Ch08 八個單元以前端穩定碼 `IMM-CH08-01` 至 `IMM-CH08-08` 對照；完成度與最高分不再依賴重複的 `topicId`。
- 「第二型過敏（血清病非II）」及半形括號舊名稱與「第二型過敏」視為同一單元，畫面統一顯示新名稱。
- 題目練習率改由實際章節題目 ID 與學生 `attemptedQuestions` 取交集計算。
- 題目尚未載入時顯示「讀取中」，讀取失敗顯示「無法讀取」，不再以 `0%` 代替未知狀態。
- 本版不修改 Firestore collection、學生歷史作答、最高分或既有 topicProgress。
- 完整調查與對照表見 `CH08_COMPATIBILITY_DECISION_2026-08-19.md`。

## v1.933 學生端科目三層導覽

- 登入後固定先顯示科目選擇，即使學生只有一個科目也不自動進入。
- 選擇科目後進入科目首頁，先顯示該科目的「目前尚待完成」入口，再列出該科目全部章節。
- 待完成入口顯示該科目未達標數量；點擊後才進入待完成單元清單，不再於科目選擇頁跨科目展開。
- 待完成清單只包含該科目中老師目前指定且最高有效分數未達門檻的單元，保留測驗與閃卡快捷入口。
- 該科目沒有待完成單元時仍保留入口並顯示「已全部完成」；點入後顯示完成提示。
- 待完成頁返回科目首頁，科目首頁可返回科目選擇；從測驗或閃卡返回時保留原本的科目與導覽層級。
- 全部章節仍包含已完成、未完成與未納入目前完成度的所有章節。
- 本版不變更 Firestore collections、GAS 資料介面或完成度計算規則。

## v1.932 目前必須完成單元看板

- 將既有 `completionTopics`／`completionTopicIds` 明確定義為「目前必須完成的單元」，老師可隨課程進度調整。
- 未勾選的未來單元不列入截至目前完成度，也不會被學生誤認為尚未練習。
- 學生入口新增精簡「目前尚待完成」清單，只列老師指定且最高有效分數未達門檻的單元。
- 每個待完成單元只顯示科目、單元、目前最高分／距達標差距，以及測驗、閃卡入口。
- 已全部完成時顯示完成訊息；入口總覽改稱「截至目前完成度」。
- 學生每次登入會強制重新讀取 `system/main`，避免沿用同一頁面中的舊完成度設定快取。
- 老師更新流程維持「儲存設定 → 同步設定」；不修改或刪除學生既有最高分，只改變目前統計範圍。
- 學生端、後台、Firebase fallback 與 GAS `APP_VERSION` 同步更新為 `v1.932`。

## v1.931 手動更新學生完成度看板

- 後台新增「完成度看板」頁籤，可選擇科目與單一單元，或查看該科目全部完成度設定單元。
- 看板合併 Google Sheet 學生名單與 Firebase `studentProgress`，因此尚未作答學生也會列出。
- 狀態分為「已完成、練習中、尚未練習」，並顯示最高有效分數與最後更新時間。
- 選擇整個科目時顯示每位學生達標單元數／該科目設定單元數；選擇單元時顯示該單元目前狀態。
- 不使用 `onSnapshot`。只有按「更新資料」時單次讀取 `studentProgress`；切換科目、單元及搜尋只篩選記憶體資料。
- 畫面會顯示本次快取的進度文件筆數，方便估算每次手動更新的 Firestore document reads。
- 讀取全班資料前會驗證 Firebase ID token 的 `admin: true` Custom Claim。
- 完成判定以目前後台 `passScore` 對最高有效分數重算，不使用可能過期的舊 `passed` 值；入口與歷次成績同步修正。
- 學生端、後台、Firebase fallback 與 GAS `APP_VERSION` 同步更新為 `v1.931`。

資料路徑：

```text
Google Sheet 學生名單／完成度設定
  → GAS getTeacherData
  → admin.html 的 teacherData

Firebase studentProgress（按更新資料時單次 get）
  → 與 teacherData.studentInfoMap 合併
  → 依學生科目ID、選定科目與完成度設定單元篩選
  → best >= 目前 passScore
  → 完成度看板
```
...
- 正式完成度唯一規則為：完整題目測驗或完整閃卡的單元最高有效分數達到後台門檻。
- 入口頂端整體完成度、單元完成狀態、歷次成績完成標籤與後台均讀取 `studentProgress`。
- 入口單元卡片原本的「完成度％」其實是做過題數比例，本版正名為「題目練習率」，避免與達標完成度混淆。
- 單元卡片新增「完成狀態：已完成／未完成／未納入完成度」。
- 歷次成績第三行及逐題明細加入相同的目前完成狀態標籤。
- 抽題練習仍可增加題目練習率，但不更新單元最高有效分數，也不會變成已完成。
- 舊學生資料若尚未設定科目ID，完成度會以目前可見題庫範圍計算，不再被誤判為 0 個單元而隱藏。
- 學生端、後台、Firebase fallback 與 GAS `APP_VERSION` 同步更新為 `v1.930`。

## v1.929 歷次成績三行精簡格式

- 每筆歷次成績固定顯示三行：科目、單元、成績。
- 作答日期保留在成績行右側，方便辨識同一單元的多次測驗。
- 答對題數、答錯題數與總用時不再占用清單空間，點入逐題明細後才顯示。
- 單元名稱維持完整換行，手機不需橫向滑動。
- 舊成績缺少科目名稱時，會依 `topicId` 或單元名稱從目前題庫補找科目；仍找不到時顯示「未設定科目」。
- 學生端、後台、Firebase fallback 與 GAS `APP_VERSION` 同步更新為 `v1.929`。

## v1.928 歷次成績表格與顯示單元設定

- 學生端歷次成績改為表格，欄位包含單元、作答時間、答對題數、用時與成績。
- 長單元名稱在表格內完整換行；手機可橫向滑動查看其餘欄位。
- 點擊表格任一列仍可開啟原有逐題作答明細。
- 後台「完成度與首頁設定」新增「歷次成績顯示單元」勾選區。
- 設定保存為 `score_history_topics` 與 `score_history_topic_ids`，再透過「同步設定」發布到 Firebase。
- 未勾選任何單元時維持相容行為：學生端顯示所有歷次成績。
- 學生端、後台、GAS `APP_VERSION` 與資源 cachebuster 同步更新為 `v1.928`。

部署後請先在後台儲存設定，再按「同步設定」，學生重新登入後即會套用顯示範圍。

## v1.927 歷次成績長單元名稱顯示

- 歷次成績清單的單元名稱由單行 `truncate` 改為最多兩行，超過兩行才省略。
- 標題容器加入 `min-width: 0` 與彈性寬度，右側分數維持固定顯示，不再把名稱擠到消失。
- 歷次成績明細頁的單元名稱同步採最多兩行，右側分數使用固定寬度配置。
- 完整單元名稱保留在 `title` 屬性，桌面端可輔助查看。
- 學生端、後台、GAS `APP_VERSION` 與資源 cachebuster 同步更新為 `v1.927`。
- 本版不變更 Firestore collections、rules 或 indexes。

## v1.926 測驗倒數與長單元名稱顯示

- 測驗題目載入完成後，依實際題數以每題 60 秒開始倒數；例如 40 題為 2,400 秒（40:00）。
- 測驗期間在頁面頂端顯示倒數，剩餘 20% 轉為警示色，最後 60 秒轉為紅色。
- 時間歸零即停止作答並自動交卷；未作答題保留「未作答」，截止後不能再修改答案。
- 手動交卷會停止計時，成績保存實際使用秒數；逾時交卷保存該次時間上限。
- 手機端章節名稱由單行省略改為最多兩行，超過兩行才顯示省略；平均秒數移到章節資訊列，避免占用標題空間。
- 學生端與後台的資源 cachebuster、可見版本及 GAS `APP_VERSION` 同步更新為 `v1.926`。
- 本版不變更 Firestore collections、rules 或 indexes。

部署順序：

1. 部署 `index.html`、`admin.html`、`firebase-config.js` 與 `firebase-v1685.js` 至網站。
2. 更新 `Code.gs`，讓後續 Firebase 同步狀態寫入 `v1.926`。
3. 以手機實機確認長單元名稱最多兩行、40 題倒數顯示 40:00、歸零自動交卷。
...
v1.933 起，學生端使用固定三層導覽。進入主選單後流程為：

```text
選科目
→ 科目首頁（目前尚待完成入口＋全部章節）
→ 可進入待完成單元清單，或直接選擇全部章節中的章節
...
未點選科目前，只顯示科目按鈕，不顯示任何章節或跨科目待完成清單。即使學生只有一個可用科目，也必須自行點選。
...
答題中不會寫入 Firebase。學生完成本次作答並按送出後，才一次寫入 `answerBatches` 與 `attemptedQuestions`；只有符合下列採計規則的作答才會更新成績摘要與達標狀態。

單元成績只採計「全部題目」測驗與閃卡複習；抽選 10／20／30／50 題及錯題重做只作為練習，不更新單元最高分或達標狀態。所有模式仍可更新已作答題目紀錄。
...
- email 不可重複。
- 學號不可重複。
- `科目ID` 必須對應已發布題庫中的 `科目ID`；多個科目可用逗號分隔。
- 學生完成度項目為「後台勾選的完成度章節」與「學生科目ID所屬章節」的交集。
- `啟用狀態` 可填 `停用`、`false`、`disabled`、`0` 來停用學生。
- 後台可按「檢查學生名單」檢查 email、學號及科目ID錯誤。
