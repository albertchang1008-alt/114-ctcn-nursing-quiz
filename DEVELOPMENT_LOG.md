...
- 完成度看板新增學生／單元視角切換，篩選、摘要與已載入資料在切換時保持不變。
- 學生視角完成度欄新增升冪／降冪切換，以完成百分比為主要排序鍵。
- 學生視角尚缺單元改為數量按鈕，點擊後以可捲動彈窗顯示該生完整缺漏清單。
- 完成度看板新增依科目產生的學生核取式複選器，支援全選、清除及任意多人；文字搜尋則在已選學生中再次篩選。
- 單元視角以目前篩選學生為分母計算完成率，支援低到高／高到低排序與同完成率同名次。
- 已完成、練習中、尚未開始以三色人數按鈕呈現，點擊後顯示依班級與學號排序的學生名單。
- 姓名預設遮蔽並以 `O` 取代中間字，可切換完整姓名；學生表、單元名單與最近 100 筆紀錄同步更新。
- 最近作答清單跟隨目前單元與學生搜尋條件；所有視角、排序、遮蔽與名單操作均為前端運算。
- 修正學生計算列漏帶 `topicResults`，導致單元排行背景運算讀取 `undefined.find` 並讓看板顯示更新失敗；另加入缺漏陣列防呆。

## 版本與驗收

- 管理端可見版本與資源 cachebuster 更新為 `v1.937`；GAS API 與 `APP_VERSION` 維持 `v1.936`，不需重新部署 GAS。
- 新增靜態測試覆蓋姓名遮蔽、單元狀態加總、低完成率優先及同率同名次。
